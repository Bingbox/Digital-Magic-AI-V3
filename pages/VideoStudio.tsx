  ...
  const handleGenerate = async () => {
    ...
    try {
      const options = {
        ...
        referenceImages: referenceAssets.length > 0 ? referenceAssets : []
      };
      const url = await GeminiService.generateVideo(`${selectedTool.name}: ${prompt}`, AIModel.IMAGE_FLASH, aspectRatio, options);
      setVisualUrl(url);
      setUser({ ...user, magicEnergy: user.magicEnergy - cost });
      
      GeminiService.saveToHistory({
        title: `视觉创作-${selectedTool.name}`,
        type: 'video', 
        content: url
      });
    } catch (err: any) {
      ...
      {isPreviewOpen && visualUrl && (
        <div className="fixed inset-0 z-[1000] bg-black/98 backdrop-blur-2xl flex flex-col animate-in fade-in duration-300" onClick={() => setIsPreviewOpen(false)}>
           <div className="px-6 py-4 flex items-center justify-between relative z-10 bg-black/40 border-b border-white/5">
             ...
             <div className="flex items-center gap-3">
                <a href={visualUrl} download="magic_visual.png" className="flex items-center gap-2 px-4 py-2 bg-white/10 hover:bg-white text-white hover:text-black rounded-lg text-[10px] font-black uppercase tracking-widest transition-all border border-white/10">
      ...
      <div className="w-full lg:w-80 flex flex-col gap-6 shrink-0 h-full overflow-hidden">
        <div className="flex-grow bg-[#0f172a]/80 border border-slate-800 rounded-xl flex flex-col backdrop-blur-xl shadow-2xl relative overflow-hidden">
          ...
          <div className="flex-grow overflow-y-auto no-scrollbar p-6 space-y-6 relative z-10 custom-scrollbar">
            ...
            <div className="space-y-3">
              <label className="text-[9px] font-black text-slate-500 uppercase tracking-widest">{t('uploadReference')} (Max 3)</label>
              <div className="grid grid-cols-3 gap-2">
                ...
                {referenceAssets.length < 3 && (
                  ...
                  >
                    ...
                    <input type="file" ref={fileInputRef} onChange={handleAssetUpload} multiple accept="image/*" className="hidden" />
                  </div>
                )}
              </div>
            </div>
          ...
          <div className="p-6 border-t border-slate-800 shrink-0 relative z-10">
            ...
            >
              {isGenerating ? (
                <div className="flex items-center gap-2 text-xs"><Loader2 className="animate-spin" size={16} /><span>Magic in progress...</span></div>
              ) : (
                <>
                  <div className="flex items-center gap-2 text-xs"><Brain size={16} /><span>{t('startMagic')}</span></div>
                  <span className="text-[8px] opacity-60 font-bold">10 {t('energy')}</span>
                </>
              )}
            </button>
          </div>
      ...
      <div className="flex-grow flex flex-col bg-[#0f172a]/40 border border-white/5 rounded-xl overflow-hidden shadow-2xl backdrop-blur-md h-full">
        <header className="px-6 py-4 border-b border-white/5 flex items-center justify-between bg-slate-900/60 shrink-0">
          <div className="flex items-center gap-3">
            ...
            {visualUrl && <span className="px-2 py-0.5 bg-rose-500/10 border border-rose-500/20 rounded text-[9px] font-black text-rose-400 uppercase tracking-widest">FLASH 2.5 READY</span>}
          </div>
          ...
          {isGenerating ? (
            <div className="flex flex-col items-center gap-6 animate-pulse text-center p-10">
                ...
                <div className="space-y-2">
                  <p className="text-sm font-black text-white uppercase tracking-widest">Generating Visual Magic...</p>
                  <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">Powered by Gemini 2.5 Flash Image</p>
                </div>
            </div>
          ) : visualUrl ? (
            <div className="w-full h-full flex flex-col items-center justify-center group relative">
               ...
               <div className="absolute inset-x-0 bottom-10 flex items-center justify-center gap-4 opacity-0 group-hover:opacity-100 transition-all duration-500 translate-y-4 group-hover:translate-y-0">
                  ...
                  <a href={visualUrl} download="magic_visual.png" className="p-3 bg-white/10 backdrop-blur-xl border border-white/10 text-white rounded-xl hover:bg-white hover:text-black transition-all shadow-xl hover:scale-110"><Download size={18} /></a>
               </div>
            </div>
          ) : (
